...
def credenciais():   
    ...
    if criar_config == 's':
        with open("config.php", "w") as f:
            ...
            f.write("$dsn = ""mysql:host={$db_host};dbname={$db_name};charset={$charset}"";\n")
            f.write("try {\n")
            f.write("    $conn = new PDO($dsn, $db_user, $db_pass);\n")
            f.write("    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);\n")
            f.write("    $conn->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);\n")
            f.write("} catch (PDOException $e) {\n")
            f.write("    // Em produção, logar o erro e mostrar mensagem genérica\n")
            f.write("     die(""Erro ao conectar ao banco de dados."");\n")
            f.write("}\n")
            f.write("?>\n") 
            f.close()
